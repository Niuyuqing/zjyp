var spsxMain = new Vue({
	...
	mounted: function() {
        ...
        if(this.getUrlParam('labelname')){  // 筛选文本
	...
	methods: {
		// 展示商品列表
		showGoodsList : function(inputStr,sortVal){
		...
		// 展示商品筛选列表
		goodsFilterList : function(inputStr){
		...
		showCityLabelClick: function(i,e) {   // 点击筛选条件更多按钮
		...
		toGoodsDetail : function(id){   // 点击商品跳转商品详情页面
		...
		priceBtnClick: function(type) { // 点击价格
			console.log(type)
			if (type==1) {   // 默认降序
