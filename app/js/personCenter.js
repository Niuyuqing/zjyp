...
var loginWrap = new Vue({
	...
	methods: {
		showOrderList: function(status) { // 展示订单列表
		...
		orderDetailFn: function(id) { // 订单详情
			window.location.href = 'orderDetail.html?oid=' + id;
		},
		openCancleBox: function(id) { // 打开取消订单窗口
			this.cancleBox = true;
		...
		cancleOrderClick: function() { // 取消订单
			if(this.cancleResult == '') { // 未选择提交理由
				alert('请选择取消订单原因');
			} else {
		...
		delBoxClick: function(val, type) { // 删除订单
			this.delBox = val;
		...
		delBoxAlert: function(val, orderId) { // 删除订单
			this.delBox = true;
		...
		confirmReceipt: function(id) {  // 确认收货
		...
		toGoodsDetail: function(id) { // 跳转到商品详情页面
