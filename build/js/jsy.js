	...
	var jsyMain = new Vue({
	  ...
	  setTimeout(function () {
		...
		methods: {
			...
			choseReceivingAddr: function choseReceivingAddr(e, i, region, address, name, phone, infoId) {
				// 选中收货地址
			...
			confirmDelCollectGoods: function confirmDelCollectGoods(type) {
				// 确认删除收货地址
				this.delBox = false;
				if (type == '1') {
					// 确定删除
	   ...
	   */
			...
			editCollectGoods: function editCollectGoods(id, name, region, addr, phone) {
				// 点击收货地址编辑
				this.editBox = true;
			...
			editConsigneeMsg: function editConsigneeMsg(val, type) {
				// 编辑收货人信息
				if (type == '2') {
					this.editBox = val;
				} else if (type == '3') {
	   ...
	   console.log(this.threeUserReceiveInfoList);*/
			...
			updateUserReceiveInfo: function updateUserReceiveInfo(id, region, name, phone, addr) {
				// 收货地址的修改
				this.$http.post('http://localhost:8083/zujahome-main/user/updateUserReceiveInfo', {
					id: id,
